    ...
    public void run() {
        try {
            while(!clt.isClosed()) {
                ...
                if (nuevoMensaje.equals("TEST")) {
                    System.out.println(" [RECIBIDO] TEST");
                    continue;
                }

                logger.info(" <---- Recibido el msg " + nuevoMensaje);

                Mensaje t = new Mensaje(nuevoMensaje, true);

                elTablero.insertarEnMensajes(t);
            }
        } catch (IOException e) {
            logger.error("Error al recibir un msg", e);
        } catch(Exception e2) {
            logger.error("Error al leer un msg", e2);
        } finally {
            try {
                clt.close();
            } catch (IOException e) { ; }
